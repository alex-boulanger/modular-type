        ...
        {CONTROLS.map((control, index) => (
          <div className="slider-control" key={control.key}>
            <div className="slider-heading">
              <label htmlFor={`style-${control.key}`} title={control.description}><span className="control-number" aria-hidden="true">0{index + 1}</span>{control.label}</label>
              <output htmlFor={`style-${control.key}`}>{String(Math.round(style[control.key] * 100)).padStart(3, '0')}</output>
            </div>
            <input id={`style-${control.key}`} type="range" min="0" max="100" step="1"
              style={{ '--range-progress': `${Math.round(style[control.key] * 100)}%` } as CSSProperties}
